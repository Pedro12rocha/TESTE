...
def woa(max_iter, n, dim, minx, maxx):
    ...
    combinacoes_kw = list(itertools.product(*matriz_kw))
    combinacoes_kvar = list(itertools.product(*matriz_kvar))
    for i, cargas in enumerate(combinacoes_kw):  # i in range(5):
        # for j in range(3):
        dss.loads_first()
        for j, carga in enumerate(cargas):  # range(dss.loads_count()):
            # print('MATRIZ_KW:',matriz_kw)
            dss.loads_write_kw(carga)  # matriz_kw[i][j])
            dss.loads_write_kvar(combinacoes_kvar[i][j])  # matriz_kvar[i][j])
            dss.loads_next()
        dss.solution_solve()
        # -----------------------------------------------------------------------------------------------------------------------
        ...
        while Iter < max_iter:
            ...
            for i in range(n):
                ...
                # then clip it
                for j in range(dim):
                    whalePopulation[i].position[j] = abs(
                        int(max(whalePopulation[i].position[j], minx)))
                    whalePopulation[i].position[j] = abs(
                        int(min(whalePopulation[i].position[j], maxx)))
                    ...
                    if whalePopulation[i].position[j] in buses:
                        ...
                        if whalePopulation[i].position[j] == "sourcebus":
                            ...
                            functions.define_3ph_pvsystem(
                                dss, whalePopulation[i].position[j],
                                mv_bus_voltage_dict[whalePopulation[i].position[j]], p_step * kva_to_kw, p_step)
                            functions.add_bus_marker(
                                dss, whalePopulation[i].position[j], "blue", 2)
                    ...
                    vdeviation_1 = (
                            (1 - abs(max_value_v_pu)) / abs(max_value_v_pu))
                    vdeviation_2 = (
                            (1 - abs(min_value_v_pu)) / abs(min_value_v_pu))
